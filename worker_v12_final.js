...
function genPrice(last, day) {
  ...
  var p=Math.max(last*0.85,last+t+(rand()-0.5)*10);if(p>1500)p+=(1500-p)*0.02;return p;
}
...
function nextDay() {
  if (simDay >= MAX_DAYS) {
    running = false;
    document.getElementById('playBtn').textContent = '✅ 完成';
    document.getElementById('playBtn').className = 'btn-pause';
    showResults();
    return;
  }
  simDay++;
...
function showResults() {
  var tip = document.getElementById('teachTip');
  var bestStrat="",bestVal=-9999;
  for(var k in strats){var v=strats[k].equity;if(v>bestVal){bestVal=v;bestStrat=k;}}
  var bhR = (priceHist[priceHist.length-1]-1000)/1000*100;
  var lines = '✅ <b>模拟结束！</b>共'+MAX_DAYS+'个交易日（≈1年）<br><br>';
  lines += '🏆 <b>最优策略：'+bestStrat+'</b> 最终资产 '+bestVal.toFixed(2)+'元<br>';
  for(var k in strats){
    var s=strats[k],ret=(s.equity-5000)/5000*100;
    lines += (k===bestStrat?'⭐ ':'&nbsp;&nbsp;')+k+'：'+(ret>=0?'+':'')+ret.toFixed(2)+'% ('+s.trades.length+'笔交易)<br>';
  }
  lines += '<br>📈 买入持有：'+(bhR>=0?'+':'')+bhR.toFixed(2)+'%<br>';
  lines += '<br>💡 <b>结论：</b>'+(bestVal/5000-1 > bhR/100 ? '好的策略能跑赢买入持有！🎯' : '这次策略没跑赢买入持有...市场随机性很大');
  tip.innerHTML = lines;
}
